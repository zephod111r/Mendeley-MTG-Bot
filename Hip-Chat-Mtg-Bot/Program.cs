...
namespace Hip_Chat_Mtg_Bot
{
    class Program
    {
        ...
        {
            HipchatMessenger.Init();
            UpdateAndLoadData(null);
            updateTimer = new Timer(UpdateAndLoadData, null, 24*60*60000, System.Threading.Timeout.Infinite);
            HipchatMessenger.Handle(regexPatternSet, setSetToUse);
            HipchatMessenger.Handle(regexPatternCard, getCard);
            HipchatMessenger.Handle(regexPatternUser, getUserProfile);
            HipchatMessenger.Handle(regexPatternVote, getVote);
        ...
        private static void DisplayRareOfTheDay(Object o)
        {
            ...
            while (!cardOfTheDayFound)
            {
                ...
                HipchatMessenger.SendMessage(cardData, RoomColors.Yellow);
        ...
        private static string getUserProfile(string userName, string requestingUser)
        {
            ...
            {
                return HipchatMessenger.GetUserPicture("@FindlayHannam");
            }
            return HipchatMessenger.GetUserPicture(userName);
        }
        ...
        private static string getVote(string vote, string requestingUser)
        {
            ...
            if (isNewVote)
            {
                if (CurrentVote != null)
                {
                    return "Current Vote is still in progress, please wait for it to finish!";
                }

                CurrentVote = new Vote(requestingUser, vote, answers, duration);
            }
            else
            {
