    ...
    class Azure
    {
        ...
        CloudTable EnsureTablePresence(string tableName)
        {
            try
            {
                ...
                Tables[tableName].CreateIfNotExistsAsync().Wait();
            }
            catch (StorageException err)
            {
                Console.Out.WriteLineAsync(err.Message);
        ...
        public bool Download(string set, string name, string rootName)
        {
            ...
            try
            {
                blockBlob.DownloadToFileAsync(rootName + "/" + set + "/" + name, System.IO.FileMode.Create).Wait();
                return true;
            }
            catch (Exception) { }
            return false;
            ...
            where typeOfEntity : TableEntity, new()
        {
            try
            {
                ...
                elements = table.ExecuteQuerySegmentedAsync<typeOfEntity>(query, new TableContinuationToken()).Result.ToList();
            }
            catch (StorageException err)
            {
                Console.Out.WriteLineAsync(err.Message);
            ...
            where typeOfEntity : TableEntity, new()
        {
            try
            {
                ...
                return table.ExecuteQuerySegmentedAsync<typeOfEntity>(query, new TableContinuationToken()).Result.FirstOrDefault();
            }
            catch (StorageException err)
            {
                Console.Out.WriteLineAsync(err.Message);
